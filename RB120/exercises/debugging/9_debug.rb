...
=begin
There is no user-defined `send` method defined within the PostalService class nor in its parent CommunicationsProvider nor anywhere in its
inheritance hierarchy. Therefore, the `send` invoked on line 91 is the instance method defined in the Object
class. This method expects to be passed methods (e.g. `:some_method`).
`ellens_postal_service.street_address` returns the string `'860 Blackbird Ln.'` Ruby tries to
interpret this string as a method and fails.

We can fix this issue by including the `Mailing` module in the `CommunicationsProvider` class.
Thus, when we call `send` on `johns_postal_service`, the custom instance method `send` defined
in the `Mailing` module will override the instance method defined in the Object class.
=end