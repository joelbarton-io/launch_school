...
=begin
The error arises because on line 60 within the `==` method, we attempt to invoke the private `serial_number`
instance method on two separate instances, in this case the object calling `serial_number` and the object
passed as an argument (referenced by the parameter `other`).  This sort of comparison is not possible from
within a private instance method.  To solve this problem, we need to make `serial_number` protected.
=end